...
static void event_handler(void *arg, esp_event_base_t event_base, int event_id, void *event_data) {
  ...
  if (event_base == WIFI_PROV_EVENT) {
    ...
    case WIFI_PROV_CRED_RECV: {
      ...
      ESP_LOGD(TAG, "Received Wi-Fi credentials\n\tSSID: %s\n\tPassword: %s", (const char *)wifi_sta_cfg->ssid,
    ...
    case WIFI_EVENT_STA_START: {
      const char *local_device_hostname = wifi_prov_mgr::get_hostname();
      if (local_device_hostname) {
        ESP_LOGI(TAG, "Setting WiFi hostname to: %s", local_device_hostname);
        tcpip_adapter_set_hostname(TCPIP_ADAPTER_IF_STA, local_device_hostname);
      }
    ...
    case IP_EVENT_STA_GOT_IP: {
      ...
      ESP_LOGI(TAG, "Connected with IP Address: %s", inet_ntoa(event->ip_info.ip));
...
const char *wifi_prov_mgr::get_hostname() { return device_hostname; }
...
void wifi_prov_mgr::init(const char *pop) {
  ...
  ESP_ERROR_CHECK(esp_event_handler_register(IP_EVENT, ESP_EVENT_ANY_ID, &event_handler, &mWifiCallback));
  ...
  if (!provisioned) {
    ...
    ESP_ERROR_CHECK(wifi_prov_mgr_start_provisioning(security, pop, get_hostname(), service_key));
  } else {
