...
void mqtt_set_on_connected_cb(mqtt_on_connected_cb_t cb) { on_connected_cb = cb; }

void mqtt_set_on_disconnected_cb(mqtt_on_disconnected_cb_t cb) { on_disconnected_cb = cb; }

static mqtt_subscription_t *mqtt_subscription_add(mqtt_subscription_t **list, const char *topic,
                                                  mqtt_on_message_received_cb_t cb, void *ctx,
                                                  mqtt_free_ctx_cb_t free_cb) {
...
static void mqtt_subscription_remove(mqtt_subscription_t **list, const char *topic) {
...
static mqtt_publications_t *mqtt_publication_add(mqtt_publications_t **list, const char *topic, uint8_t *payload,
                                                 size_t len, int qos, uint8_t retained) {
  mqtt_publications_t *pub = (mqtt_publications_t *)malloc(sizeof(*pub));

  pub->topic = strdup(topic);
  pub->payload = (uint8_t *)malloc(len);
...
static void mqtt_publications_publish(mqtt_publications_t *list) {
  for (; list; list = list->next) {
    ESP_LOGI(TAG, "Publishing from queue: %s = %.*s", list->topic, list->len, list->payload);

    mqtt_publish(list->topic, list->payload, list->len, list->qos, list->retained);
  }
}

int mqtt_subscribe(const char *topic, int qos, mqtt_on_message_received_cb_t cb, void *ctx,
                   mqtt_free_ctx_cb_t free_cb) {
  if (!is_connected)
    return -1;
...
int mqtt_publish(const char *topic, uint8_t *payload, size_t len, int qos, uint8_t retained) {
  if (is_connected) {
    return esp_mqtt_client_publish(mqtt_handle, (char *)topic, (char *)payload, len, qos, retained) < 0;
  }

  /* If we're currently not connected, queue publication */
  ESP_LOGD(TAG, "MQTT is disconnected, adding publication to queue...");
  mqtt_publication_add(&publications_list, topic, payload, len, qos, retained);

  return 0;
}

static void mqtt_message_cb(const char *topic, size_t topic_len, uint8_t *payload, size_t len) {
  mqtt_subscription_t *cur;

  ESP_LOGD(TAG, "Received: %.*s => %.*s (%d)\n", topic_len, topic, len, payload, (int)len);

  for (cur = subscription_list; cur; cur = cur->next) {
    /* TODO: Correctly match MQTT topics (i.e. support wildcards) */
    if (strncmp(cur->topic, topic, topic_len) || cur->topic[topic_len] != '\0') {
      continue;
    }
...
static esp_err_t mqtt_event_cb(esp_mqtt_event_handle_t event) {
  ...
  case MQTT_EVENT_DATA:
    mqtt_message_cb(event->topic, event->topic_len, (uint8_t *)event->data, event->data_len);
    break;
  default:
    break;
  }
...
int mqtt_connect(const char *host, uint16_t port, const char *client_id, const char *username, const char *password,
                 uint8_t ssl, const char *server_cert, const char *client_cert, const char *client_key) {
  esp_mqtt_client_config_t config = {
      .event_handle = mqtt_event_cb,
      .host = Utilities::resolve_host(host),
      .port = port,
      .client_id = client_id,
      .username = username,
      .password = password,
...
uint8_t mqtt_is_connected(void) { return is_connected; }
