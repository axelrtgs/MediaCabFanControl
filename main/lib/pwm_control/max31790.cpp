...
esp_err_t MAX31790::readFanConfig(const uint8_t &index, FANCONFIG *fanConfig) {
  ...
  fanConfig->tachInputEnable =
      static_cast<Tach_Input_Enable>(data & static_cast<uint8_t>(FanConfigMask::Tach_Input_Enable));
...
esp_err_t MAX31790::writeFanConfig(const uint8_t &index, const FANCONFIG &fanConfig) {
  ...
  return _i2c->writeByte(_deviceAddress, static_cast<uint8_t>(FanConfig[index]), data);
}
...
esp_err_t MAX31790::readFanOptions(FANOPTIONS *fanOptions) {
  ...
  fanOptions->fanFaultQueue =
      static_cast<Fan_Fault_Queue>(data & static_cast<uint8_t>(FanOptionsMask::Fan_Fault_Queue));
  fanOptions->fanFailOptions =
      static_cast<Fan_Fail_Options>(data & static_cast<uint8_t>(FanOptionsMask::Fan_Fail_Options));
  fanOptions->seqStartDelay =
      static_cast<Seq_Start_Delay>(data & static_cast<uint8_t>(FanOptionsMask::Seq_Start_Delay));
...
esp_err_t MAX31790::writeTachTargetRaw(const uint8_t &index, uint16_t data, const Speed_Range &speedRange) {
  ...
  data = Utilities::clamp_val(data, FAN_TACH_MIN, FAN_TACH_MAX) << TACH_SHIFT_BITS;
