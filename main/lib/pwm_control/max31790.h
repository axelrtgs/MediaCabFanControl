...
public:
  ...
  const Register FanConfig[NR_CHANNEL] = {Register::Fan1_Config, Register::Fan2_Config, Register::Fan3_Config,
                                          Register::Fan4_Config, Register::Fan5_Config, Register::Fan6_Config};

  const Register FanDynamics[NR_CHANNEL] = {Register::Fan1_Dynamics, Register::Fan2_Dynamics, Register::Fan3_Dynamics,
                                            Register::Fan4_Dynamics, Register::Fan5_Dynamics, Register::Fan6_Dynamics};

  const Register Tach[NR_CHANNEL * 2] = {Register::Tach1, Register::Tach2,  Register::Tach3,  Register::Tach4,
  ...
  const Register PWMDuty[NR_CHANNEL] = {Register::PWM_Duty1, Register::PWM_Duty2, Register::PWM_Duty3,
                                        Register::PWM_Duty4, Register::PWM_Duty5, Register::PWM_Duty6};

  const Register PWMTarget[NR_CHANNEL] = {Register::PWM_Target1, Register::PWM_Target2, Register::PWM_Target3,
                                          Register::PWM_Target4, Register::PWM_Target5, Register::PWM_Target6};

  const Register TachTarget[NR_CHANNEL] = {Register::Tach_Target1, Register::Tach_Target2, Register::Tach_Target3,
                                           Register::Tach_Target4, Register::Tach_Target5, Register::Tach_Target6};

  const Register Window[NR_CHANNEL] = {Register::Window1, Register::Window2, Register::Window3,
                                       Register::Window4, Register::Window5, Register::Window6};
  ...
  enum class Watch_Dog_Status : const uint8_t { I2C_Occured = 0b0 << 0, I2C_Exceed_Watchdog = 0b1 << 0 };
  ...
  enum class Oscillator_Select : const uint8_t { Internal = 0b0 << 3, External = 0b1 << 3 };

  enum class Bus_Timeout : const uint8_t { Enabled = 0b0 << 5, Disabled = 0b1 << 5 };
  ...
  enum class PWM_FreqMask : const uint8_t { PWM1_3 = 0b1111 << 0, PWM4_6 = 0b1111 << 4 };
  ...
  enum class Output_Mode : const uint8_t { PWM_Output = 0b0 << 0, Tach_Input = 0b1 << 0 };

  enum class Locked_Rotor_Polarity : const uint8_t { Low = 0b0 << 1, High = 0b1 << 1 };

  enum class Tach_Input_Mode : const uint8_t { Tach_Count = 0b0 << 2, Locked_Rotor = 0b1 << 2 };

  enum class Tach_Input_Enable : const uint8_t { Disabled = 0b0 << 3, Enabled = 0b1 << 3 };

  enum class Control_Monitor : const uint8_t { Control = 0b0 << 4, Monitor_Only = 0b1 << 4 };

  enum class Spin_Up : const uint8_t { None = 0b00 << 5, Sec_0_5 = 0b01 << 5, Sec_1 = 0b10 << 5, Sec_2 = 0b11 << 5 };
  ...
  enum class Asym_ROC : const uint8_t {
    Enabled = 0b0 << 1, // Same rate of change whether duty cycle is increasing or decreasing.
    Disabled = 0b1 << 1 // Rate of change when duty cycle is decreasing is half the rate when increasing.
  };
  ...
  esp_err_t writeFanDynamics(const uint8_t &index, const FANDYNAMICS &fanDynamics);
