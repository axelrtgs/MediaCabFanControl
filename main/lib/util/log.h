...
static int log_vprintf(const char *fmt, va_list l) {
  ...
  if ((ret = sendto(sock, buf, MIN(ret, sizeof(buf) - 1), 0, (struct sockaddr *)&dst, sizeof(dst))) < 0) {
...
int log_start(const char *host, uint16_t port) {
  ...
  if (!inet_pton(AF_INET, Utilities::resolve_host(host), &dst.sin_addr)) {
  ...
  if (IN_MULTICAST(ntohl(dst.sin_addr.s_addr))) {
    uint8_t ttl = 2;
    if (setsockopt(sock, IPPROTO_IP, IP_MULTICAST_TTL, &ttl, sizeof(uint8_t))) {
