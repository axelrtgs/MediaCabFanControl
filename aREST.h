...
const uint8_t NUMBER_ANALOG_PINS = 16;
const uint8_t NUMBER_DIGITAL_PINS = 54;
const uint16_t OUTPUT_BUFFER_SIZE = 2000;
#elif defined(__AVR_ATmega328P__) && !defined(ADAFRUIT_CC3000_H)
const uint8_t NUMBER_ANALOG_PINS = 6;
const uint8_t NUMBER_DIGITAL_PINS = 14;
const uint16_t OUTPUT_BUFFER_SIZE = 350;
#elif defined(ADAFRUIT_CC3000_H)
const uint8_t NUMBER_ANALOG_PINS = 6;
const uint8_t NUMBER_DIGITAL_PINS = 14;
const uint16_t OUTPUT_BUFFER_SIZE = 275;
#else
const uint8_t NUMBER_ANALOG_PINS = 6;
const uint8_t NUMBER_DIGITAL_PINS = 14;
const uint16_t OUTPUT_BUFFER_SIZE = 350;
...
// Hardware data
#if defined(ESP8266)
const String HARDWARE = "esp8266";
#elif defined(ESP32)
const String HARDWARE = "esp32";
#else
const String HARDWARE = "arduino";
#endif

// Size of name & ID
const uint8_t NAME_SIZE = 20;
const uint8_t ID_SIZE = 10;

// Subscriptions
const uint8_t NUMBER_SUBSCRIPTIONS = 4;
...
//const bool DISABLE_VARIABLE_ENDPOINT     = true;
const bool DISABLE_PINS_ENDPOINT         = true;
const bool DISABLE_DIGITAL_ENDPOINT      = true;
const bool DISABLE_MODE_ENDPOINT         = true;
const bool DISABLE_ANALOG_ENDPOINT       = true;
//const bool DISABLE_FUNCTION_ENDPOINT     = true;
...
// Default number of max. exposed variables
  ...
  const uint8_t NUMBER_VARIABLES = 15;
  #else
  const uint8_t NUMBER_VARIABLES = 5;
  #endif
#endif
...
// Default number of max. exposed functions
  ...
  const uint8_t NUMBER_FUNCTIONS = 10;
  #else
  const uint8_t NUMBER_FUNCTIONS = 5;
  #endif
#endif
...
void setKey(String proKey, PubSubClient& client) {
  ...
  // Build topics IDs
  String inTopic = id + proKey + String("_in");
  String outTopic = id + proKey + String("_out");

  strcpy(in_topic, inTopic.c_str());
  strcpy(out_topic, outTopic.c_str());

  // Build client ID
  String clientId = id + proKey;
}
...
void handle_callback(PubSubClient& client, char* topic, byte* payload, unsigned int length) {
  ...
  // Process aREST commands
    ...
    // Send response
      ...
      Serial.println(client_id.legth());
    #endif

    int max_message_size = 128 - 20 - client_id.legth());
...
void addToBuffer(double toAdd, bool quotable){
  char number[10];
  dtostrf(toAdd, 5, 2, number);
